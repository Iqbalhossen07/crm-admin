...
const Sidebar = ({ isOpen, setIsOpen }) => {
  ...
  const menuItems = [
    ...
    { label: 'Payments', path: '/payments', icon: 'fa-solid fa-file-invoice-dollar' },
    { label: 'Team', type: 'header' },
    { label: 'Developers', path: '/developers', icon: 'fa-solid fa-laptop-code' },
  ];
    ...
    <>
      ...
      `}>
        ...
        <div className="flex items-center justify-between gap-4 px-8 py-7">
          ...
          <button onClick={() => setIsOpen(false)} className="lg:hidden w-10 h-10 flex items-center justify-center bg-gray-50 rounded-xl text-gray-400">
        ...
        <nav className="flex-1 space-y-2 overflow-y-auto custom-scrollbar px-4 pb-10">
          {menuItems.map((item, index) => {
            // 🔥 এইখানে আমরা চেক করছি পাথটা ওই মেনুর পাথ দিয়ে শুরু হচ্ছে কি না
            const isActive = item.path === '/' 
              ? activePath === '/' 
              : activePath.startsWith(item.path);

            return item.type === 'header' ? 
            <p key={index} className="text-[10px] text-gray-400 font-black uppercase tracking-[0.2em] mt-8 mb-4 px-5">{item.label}</p> :
            <Link 
              key={index} 
              to={item.path} 
              onClick={() => setIsOpen(false)} 
        ...
        <div className="mt-auto p-6 border-t border-gray-50 bg-gray-50/30">
          <Link to="/logout" className="flex items-center gap-5 px-6 py-4 text-sm font-bold text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-2xl transition-all group">
      ...
      <div className="lg:hidden fixed bottom-5 left-1/2 -translate-x-1/2 w-[92%] max-w-[400px] bg-white/90 backdrop-blur-xl border border-gray-100 px-2 py-2.5 z-[50] flex items-center justify-around rounded-[1.5rem] shadow-[0_15px_40px_rgba(0,0,0,0.12)]">
        {menuItems.filter(i => i.path).slice(0, 5).map((item, index) => {
          ...
          return (
            ...
            >
              <i className={`${item.icon} text-[16px] ${isActive ? "scale-110" : ""}`}></i>
              <span className="text-[7px] font-black uppercase tracking-wider mt-1">{item.label.substring(0, 4)}</span>
            </Link>
          )
        })}
        {/* Full Menu Button */}
        <button onClick={() => setIsOpen(true)} className="flex flex-col items-center justify-center min-w-[50px] text-gray-400 active:scale-90 transition-transform">
          ...
          <span className="text-[7px] font-black uppercase tracking-wider mt-1">Menu</span>
        </button>
      </div>
    </>
  );
